...
describe('1 - Adicione uma cor de fundo específica à página', () => {
  ...
  it('Possuir cor de fundo: rgb(0, 0, 0)', () => {
    cy.get('body')
...
describe('4 - Adicione uma lista de metas e habilidades à página', () => {
  beforeEach(() => {
    cy.visit('/');
  });

  it('A lista deve ser numerada e possuir o ID "metasehabilidades"', () => {
    shouldExist('ol#metasehabilidades');
  });

  it('A lista deve possuir 5 itens', () => {
    cy.get('ol#metasehabilidades').find('li').should('have.length', 5);
  });
});

describe('5 - Crie uma lista de habilidades desenvolvidas', () => {
  beforeEach(() => {
    cy.visit('/');
  });

  it('A lista não deve ser numerada e deve possuir o ID "habilidades"', () => {
    shouldExist('ul#habilidades');
  });

  it('A lista deve possuir 5 itens', () => {
    cy.get('ul#habilidades').find('li').should('have.length', 5);
  });
});
...
describe('8 - Crie um artigo sobre seu aprendizado', () => {
  ...
  it('O artigo deve ter mais de 300 caracteres e menos de 1500', () => {
    cy.get('article')
      .invoke('text')
      .then((text) => {
        expect(text).to.have.length.of.at.most(1500);
